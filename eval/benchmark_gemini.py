...
def calculate_iou(bbox_gt_xywh: list, bbox_pred_yxyx_norm: list, img_width: int, img_height: int) -> Optional[float]:
    ...
    """
    if bbox_gt_xywh is None or bbox_pred_yxyx_norm is None or len(bbox_gt_xywh) != 4 or len(bbox_pred_yxyx_norm) != 4:
        return None
    try:
...
async def call_gemini_api_async(client: genai.Client, model_name: str, prompt: str, image_path: str,
                                 semaphore: asyncio.Semaphore, thinking: bool = False) -> tuple[str, Optional[str]]:
        ...
        try:
            ...
            if config:
                response = await loop.run_in_executor(
                    None, lambda: client.models.generate_content(model=model_name, contents=contents, config=config))
            else:
                response = await loop.run_in_executor(
                    None, lambda: client.models.generate_content(model=model_name, contents=contents))
...
async def process_single_item(idx: int, item: dict, client: genai.Client, model_name: str, image_root: str,
                               semaphore: asyncio.Semaphore, logger: logging.Logger,
                               max_retries: int = MAX_RETRIES, thinking: bool = False) -> dict:
    image_filename = item.get('image_path', '')
    question = item.get('question', '')
    gt_bbox = item.get('bbox')
    full_image_path = get_image_path(image_filename, image_root)

    result_item = {**item, 'prediction': None, 'pred_bbox': None, 'iou': None, 'score': False,
                    'raw_response': '', 'retries': 0}
    ...
    try:
        ...
        for attempt in range(max_retries + 1):
            raw_response, thinking_summary = await call_gemini_api_async(client, model_name, prompt, full_image_path, semaphore, thinking)
            ...
            if raw_response.startswith("[ERROR]:"):
                if attempt < max_retries:
                    ...
                    logger.debug(f"[{idx}] API error, retry {attempt + 1}/{max_retries} after {wait_time}s"
...
async def run_benchmark_async(
    ...
    test_samples: Optional[int] = None, model_name: str = MODEL_NAME, concurrency: int = DEFAULT_CONCURRENCY,
    max_retries: int = MAX_RETRIES, resume: bool = False, thinking: bool = False,
) -> dict:
    ...
    async def process_and_save(idx, item):
        result = await process_single_item(idx, item, client, model_name, image_root, semaphore, logger, max_retries, thinking)
        results[idx] = result
...
Examples:
  ...
  python benchmark_gemini.py --model gemini-3-pro-preview --thinking
    ...
    parser.add_argument('--thinking', action='store_true', help='Enable thinking mode (thinking_budget=8000)')
    ...
    stats = run_benchmark(json_path, image_root, args.output, api_key, args.test_samples, args.model,
                           args.concurrency, args.max_retries, args.resume, args.thinking)
