...
async def call_gpt_api_async(client: AsyncOpenAI, prompt: str, image_path: str, model: str,
                              semaphore: asyncio.Semaphore, reasoning_effort: Optional[str] = None) -> tuple[str, Optional[int]]:
...
async def process_single_item(client: AsyncOpenAI, item: dict, image_root: str, model: str,
                               semaphore: asyncio.Semaphore, idx: int, total: int, logger: logging.Logger,
                               reasoning_effort: Optional[str] = None, max_retries: int = MAX_RETRIES) -> dict:
    ...
    if not os.path.exists(full_image_path):
        ...
        return {**item, 'prediction': None, 'pred_bbox': None, 'iou': None, 'score': False,
    ...
    for attempt in range(max_retries):
        try:
            response_text, reasoning_tokens = await call_gpt_api_async(client, prompt, full_image_path, model, semaphore, reasoning_effort)
            ...
            result_item = {**item, 'prediction': prediction, 'pred_bbox': pred_bbox, 'iou': iou,
                            'score': compute_score(prediction, item.get('answer')), 'raw_response': response_text,
                            'retries': attempt}
            if reasoning_tokens:
    ...
    return {**item, 'prediction': prediction, 'pred_bbox': pred_bbox, 'iou': iou,
            'score': compute_score(prediction, item.get('answer')), 'raw_response': last_response or '',
            'retries': max_retries, 'retry_reasons': retry_reasons, 'error': f"All {max_retries} retries failed"}
...
async def run_benchmark_async(
    json_path: str, image_root: str, output_path: str, api_key: str,
    test_samples: Optional[int] = None, model: str = MODEL_NAME, concurrency: int = DEFAULT_CONCURRENCY,
    max_retries: int = MAX_RETRIES, resume: bool = False, reasoning_effort: Optional[str] = None,
) -> dict:
    ...
    if resume and os.path.exists(output_path):
        ...
        existing_results = existing_data['results'] if isinstance(existing_data, dict) and 'results' in existing_data else existing_data
        failed_indices = [i for i, r in enumerate(existing_results) if r is None or 'error' in r or r.get('prediction') is None]
    ...
    metadata = {'model_name': model, 'input_json': json_path, 'test_datetime': datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    save_lock = asyncio.Lock()

    async def process_and_save(idx, item):
        result = await process_single_item(client, item, image_root, model, semaphore, idx, len(data), logger, reasoning_effort, max_retries)
        results[idx] = result
    ...
    return {'total': len(data), 'mcq_accuracy': mcq_stats['accuracy'], 'mcq_correct': mcq_stats['correct'],
            ...
            'elapsed_time': round(elapsed, 1), 'output_path': output_path, 'log_path': log_file}
...
Examples:
  ...
  python benchmark_gpt.py --model gpt-5.2 --reasoning_effort high
    ...
    parser.add_argument('--model', type=str, default=MODEL_NAME, help=f'GPT model name (default: {MODEL_NAME})')
    ...
    parser.add_argument('--reasoning_effort', type=str, default=None, choices=['low', 'medium', 'high', 'xhigh'],
    ...
    stats = run_benchmark(json_path, image_root, args.output, api_key, args.test_samples, args.model,
                           args.concurrency, args.max_retries, args.resume, args.reasoning_effort)
